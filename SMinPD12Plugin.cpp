...
int SMinPD12Plugin::is_break( char ch ){
	if( ch == '\n' || ch == '\r' || ch == '\n' ){
		return 1;
	}
	else{
...
void SMinPD12Plugin::freeBKPNode(BKPNode *bkpLL)
{
	BKPNode *Navigate;
	while(bkpLL!=NULL){
		Navigate=bkpLL;
...
int SMinPD12Plugin::BitCriteria(int Num, int Bit){/* Is bit set in Num */
	int i;
	int PowBit;
...
int SMinPD12Plugin::TwoCrossovers(int Num, int MaxBit, int *Begin, int *End){/* Is bit set in Num */
	int i;
	int Start,Stop;
	Stop=-1;
	Start=-1;
...
double SMinPD12Plugin::SumofSquaredValues(double ***values, int **values2, int points, int data, int s,double *m_x)
{
	double Sx;
	int t;
		(*m_x)=0;
...
char ** SMinPD12Plugin::MatrixInit(int s, int dim1, int dim2)
{
	char  **matrix  = NULL;
	int   i;
...
/*********************************************************************/
double SMinPD12Plugin::JC69distance(int gaps, int matches, int al_len, double alpha)
{
...
/*********************************************************************/
double SMinPD12Plugin::K2P80distance(int p1, int p2, int gaps, int matches, int al_len, double alpha)
{
...
/*********************************************************************/
double SMinPD12Plugin::TN93distance(int pT, int pC, int pG, int pA, int p1, int p2, int gaps,int matches, int al_len, double alpha, int s, int t)
{
...
/*  					Calculate pairwise			  							          */
/******************************************************************************************/
double SMinPD12Plugin::PairwiseDistance(char **seqsChars,int t, int s, int windowSize, int model)
{
	int i;
	char c1,c2;
...
/*  																  */
/**********************************************************************/
int SMinPD12Plugin::DistOnly(Fasta **seqs, double **dist, double ***dist_frags, int maxDim, int n,int win_Count, int model, int windowSize, int step, int numWins)
{
	char		**seqChars;
...
/*  																  */
/**********************************************************************/
int SMinPD12Plugin::SaveMinResults(Mintaxa **minresults, int *resno, int a, int c, double dist, double div)
{
...
/*  																  */
/**********************************************************************/
void SMinPD12Plugin::OptionalRecPrintout(FILE *fp4,  RecRes *recRes,Fasta **seqs,int *Frag_Seq,FDisNode (*recSolutions)[3][2], int i, int l, int r, int p, int q, int f, int index, int last, int BKP, int win_Count,int align,int fr_size)
{
	double d_min;
	d_min=INF9999;
...
/* 							FillArrayInOrder					      */
/**********************************************************************/
double SMinPD12Plugin::FillArrayInOrder(int *arrayInOrder, double ***dist_frags, double **bootVals, int  *Frag_Seq, int s, int k, int noSeqs, int bootOpt)
{
	int t,j,p,q,i;
...
BKPNode * SMinPD12Plugin::ArrayToBKPNode( int  *F, int win_Count, int fr_size, int align, int bootOpt, int step)
{
...
/*  																  */
/**********************************************************************/
BKPNode * SMinPD12Plugin::GetRecSolutionsII( double ***dist_frags, double **bootVals, Fasta **seqs, int  *Frag_Seq, int noSeqs, int s, int win_Count, int min_seq, int fr_size, double min_seq_dist, int align, int bootOpt, int step, int *avgBS)
{
...
/**********************************************************************/
double SMinPD12Plugin::FindBestinStretch( double ***dist_frags, double **bootVals, int  *Frag_Seq,  int bootOpt,int noSeqs, int s, int win_Count, int candidate,int lb,int ub,double largest)
{
	double d;
	int i;
			d=0;
...
/*  																  */
/**********************************************************************/
BKPNode * SMinPD12Plugin::GetRecSolutionsI(double ***dist_frags, double **bootVals, Fasta **seqs, int  *Frag_Seq, int noSeqs, int s, int win_Count, int min_seq, int fr_size, double min_seq_dist, int align, int bootOpt, int step,int *avgBS)
{
...
/*  																  */
/**********************************************************************/
int SMinPD12Plugin::PickSeqofLargerAvgDis(double ***dist_frags, int k_idx, int f_idx, int min_idx,  int s, int win_Count, int k, int f,double min_d)
{
...
int SMinPD12Plugin::PickSeqMinDist(double ***dist_frags, int  *Frag_Seq, int min_idx,int noSeqs, int k_idx, int f_idx, int s, int win_Count, int k, int f,double min_d)
{
...
/* 						Print Fragments							      */
/**********************************************************************/
void SMinPD12Plugin::PrintFragments(FILE *fp3, int  *Frag_Seq, Fasta **seqs, double **dist, double ***dist_frags, int win_Count, int i, int s, int align )
{
	int t,k;
...
/**********************************************************************/
int SMinPD12Plugin::PickCandidates( Fasta **seqs, int  *Frag_Seq, double ***dist_frags, int win_Count,
...
BKPNode * SMinPD12Plugin::Check4Recombination(int  *Frag_Seq,  Fasta **seqs, double **dist, double ***dist_frags, int startAS, int s, int win_Count,  int min_idx,  int align, int maxDim, int opt, int step, int windowSize , short int bootReps, double min_d)
{
	int i, dummy;
...
double SMinPD12Plugin::GetBootstrapValues(Fasta **seqs, double ***bootVals,  int s, int a, int bootreps)
{
...
double SMinPD12Plugin::GetClusteredBootstrapValues(Mintaxa *mintaxa, int max_times, Fasta **seqs, double ***bootVals,  int s, int min_idx, int bootreps)
{
...
/******************************************************************************/
char* SMinPD12Plugin::RNACodonToDNACodon(char* RNACodon){
	int i;
	char* DNACodon;
...
/***************************************************************************/
char SMinPD12Plugin::CodonToAminoAcid(char* Codon){
	int i;
	if(RNA){
...
/**********************************************************************/
int SMinPD12Plugin::CheckClustering(Mintaxa *mintaxa,int max_times,Fasta **seqs,int s,int min_idx)
{
	int i,j,k,c,p;
...
/**********************************************************************/
void SMinPD12Plugin::StoreBootstrapValues(int *boots, Mintaxa **minresults, int resno, int start)
{
	int i,j;
...
/*  																  */
/**********************************************************************/
int SMinPD12Plugin::OutputResults12(int win_Count, int s, int startAS, double iBase, double min_d, int min_idx, int *times_max, double *div, BKPNode *bkpLL, double ***bootVals, Mintaxa *mintaxa,Mintaxa **minresults, Fasta **seqs, double **dist, int maxDim, int n, FILE *fp1, double mutrate, int fr_size, int *resno, int align, int avgBS,int bootreps)
{
	double	 d,  interval;
	int		i,  j, t,times; 
...
/*  																  */
/**********************************************************************/
int SMinPD12Plugin::GetMinDist12(Mintaxa **minresults, Fasta **seqs, double **dist, double ***dist_frags, int maxDim, int n, char *File1, char *File2, double mutrate, int win_Count, int *resno, int align, int opt, int step, int winSize)
{
	...
	*/
	...
	/* Then write distances to file */
	 for (s;s<=n;s++){
		...
		if(OutputResults12(win_Count,s,startAS,iBase,min_d,min_idx,&times_max,div,bkpLL,NULL,&mintaxa[0], minresults,seqs,dist,maxDim,n,fp1,mutrate,fr_size,resno,align,dummy,0)==false){
			return(false);
		}
		else{
...
void SMinPD12Plugin::BuildPartialNJTrees(Fasta **seqs, Mintaxa **minresults, int resno, double **dist, char *File1, int n)
{
	FILE	*fp1;
...
//fprintf(stderr,";\n\n");
	...
	while(r<resno){
		...
		if(ch==2){
			...
			if (!fp1) printf("FP1\n");
			if (!new_root) printf("NEWROOT\n");
			if (!seqs) printf("SEQS\n");
			if (!boots) printf("BOOTS\n");
...
/******************************************************************************************/
int SMinPD12Plugin::PrepareWeights(int *wmod,int bootreps, int windowSize)
{
	double rnd;
...
int SMinPD12Plugin::BootRip(short int bootreps,  int model, double alpha, int noSeqs, int windowSize, char *seqs,  double *dist,  int *wmod)
{
...
int SMinPD12Plugin::GetBootNJdistances(int bootreps,int noSeqs, double *dstMat,Fasta **seqs, int distPenalty)
{
...
int SMinPD12Plugin::DoBootscan(double ***dist_frags,int **plotVal,int numWins, Fasta **seqsBS,  int maxDim, int noSeqs,int stepSize, int windowSize,int closestRel, int opt, short int bootreps, int fullBootscan)
{
	//short int *seqsInt;
	short int full;
	char *seqChars;
...
/******************************************************************************************/
 ...
 int SMinPD12Plugin::BootscanShrinkPool(Fasta **seqs, int  *Frag_Seq, int step, int windowSize , short int bootReps, int numSeqs, int s, int numWins, int maxDim, int bootThreshold, int spikeLen)
{
...
BKPNode * SMinPD12Plugin::BootscanCandidates(Fasta **seqs, int  *Frag_Seq, int step, int windowSize , short int bootReps,int numSeqs, int s, int numWins, int min_seq, double min_seq_dist, int align, int maxDim, int opt, int *avgBS)
{
...
/******************************************************************************************/
int SMinPD12Plugin::CopyToSeqsBootscan(Fasta **seqsBS, Fasta **seqs,int numSeqs, int seqNo, int *maxBSDim)
{
	//int i=0;
...
int SMinPD12Plugin::GetParentsFromAnswer(Fasta **seqs,char *temp,int *parents, int n)
{
	int k, i,p;
	char name[20];
...
int SMinPD12Plugin::Parents_Missed(int *Frag_Seq,int numSeqs,int *parents,int p)
{
	int i,j,pa[10],k;
	for(i=0;i<p;i++)
		pa[i]=false;
...
int SMinPD12Plugin::GetAvgBootstrapDist(Fasta **seqs,double **dist, double ***bootVals, int n, int bootreps)
{
...
int SMinPD12Plugin::Bootscan(char *File1, char *File2, double ***dist_frags, int numWins, Mintaxa **minresults,Fasta **seqs, double **dist, int maxDim, int n,int step, int windowSize, int closestRel, int *resno, short int bootreps, int align) 
{
...
//		}
		...
		for (s;s<n;s++){
			...
			if(OutputResults12(numWins,s,startAS,0.0,min_d,min_idx,&times_max,NULL,bkpLL,bootVals,&mintaxa[0], minresults,seqs,dist,maxDim,n,fp1,0.0,maxDim/numWins,resno,align,avgBS,bootreps)==false)
				return(false);
			freeBKPNode(bkpLL);
		}
...
/**************************************************************/
void SMinPD12Plugin::ReadUntil(FILE *fv, char stopChar, char *what)
{
	char ch;
...
void SMinPD12Plugin::PrintUsage()
{
...
/**************************************************************/
void SMinPD12Plugin::ReadParams12( char *inputFile, char *outputFile, int *opt, int *align,  int *wSize, int *stSize, char* ifile)
{
	int  j;
...
void SMinPD12Plugin::CheckTimes(Fasta **seqs, int n){
int i, time;
time=seqs[0]->time;
...
void SMinPD12Plugin::input(std::string infile) {
	GOP=5; //+ log(len2);
	GEP=1;
	match = 2;
	mismatch=0;
...
void SMinPD12Plugin::run() {
	...
	if( 1 <= Cluster && Cluster <= 3 ){
		...
		if( GetMinDist12(minresults, seqs,dist,dist_frags, maxDim, n,OutputFileName,DistanceFileName,mutrate,fr_Dim, &res_no, align,crossOpt,stSize,wSize)!=true){
...
void SMinPD12Plugin::output(std::string outfile) {
	res_no=1;
	...
	if(opt>1){/*Sliding Window analysis*/
		...
		if(GetMinDist12(minresults, seqs,dist, 
...
PluginProxy<SMinPD12Plugin> SMinPD12PluginProxy = PluginProxy<SMinPD12Plugin>("SMinPD12", PluginManager::getInstance());


/* Convert any random array to heap array*/
void SMinPD12Plugin::heapify_mintaxa ( Mintaxa **list , int newnode )
{
	int done=0, dad;
	Mintaxa *temp;
...
/* Heap sort*/
void SMinPD12Plugin::heapsort_mintaxa ( Mintaxa **list, int last )
{
	int i;
	Mintaxa *temp;
	while(last > 0 )
...
/* Convert any random array to heap array*/
void SMinPD12Plugin::heapify_Fasta ( Fasta **list , int newnode )
{
	int done=0, dad;
	Fasta *temp;
...
/* Heap sort*/
void SMinPD12Plugin::heapsort_Fasta ( Fasta **list, int last )
{
	int i;
	Fasta *temp;
	while(last > 0 )
...
/*  																  */
/**********************************************************************/
void SMinPD12Plugin::AddTipOrNode(int *w,int *p,int *max_w, int chidx, double brlen,TNode **NodeStorage, int Join)
{
	int i;
	TNode *NewNode;
		if(*w<0||(*p<0&&Join>0))
...
/**********************************************************************/
double SMinPD12Plugin::GetAvgBrLen(TNode *node, int *nodes)
{
	double lenL, lenR;
...
/*  																  */
/**********************************************************************/
int SMinPD12Plugin::MatricizeTopology(TNode *node, int *topoMatrix, int n, int **dist, double avgBrLen,int *max)
{
...
/*  																  */
/**********************************************************************/
void SMinPD12Plugin::PrintTree( FILE *fv, TNode *node, Fasta **seqs, int *boots, int printboot )
{
	if ( node->nodeNo >= 1000 ) {
		fputc( '(', fv );
...
void SMinPD12Plugin::NJTree(TNode **NodeStorage, double ** DistMatrix, int UB, int OutgroupIn0, int *seqIds, int *max_w, int *w)
{
...
int SMinPD12Plugin::remove_old_root( TNode* root_node, TNode* old_root ){
...
void SMinPD12Plugin::reverse( TNode* old_root, TNode* node, TNode* old_child, double new_length ){
...
TNode* SMinPD12Plugin::find_parent( TNode* root_node, TNode* child_node ){
	TNode* temp_node;
...
void SMinPD12Plugin::print_nodes( TNode* node, int level, Fasta** seqs ){
	int i = 0;
	printf( "\n" );
	printf( "(%d)", level );
	for( i = 0; i < level; i++ ){
...
TNode* SMinPD12Plugin::find_node( TNode* node, int id ){
...
int SMinPD12Plugin::parse_markers_file( char* file_name, int** codon_list, int* number_of_codons ){
...
void SMinPD12Plugin::SetSeed(unsigned long seed)
{
